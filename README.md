...
A library for parsing MHAS packets.

For additional information, please have a look at the [Wiki](https://github.com/Fraunhofer-IIS/mmtmhasparserlib/wiki/home).

## Supported architectures

mmtmhasparserlib is built and tested using the following architectures and toolchains:
...
mmtmhasparserlib uses [CMake](https://cmake.org/) (version >= 3.16) to configure and build the project. A working CMake installation is therefore required to build the software.

The following project-specific CMake parameters are available:

<table>
<tr>
<td><code>mmtmhasparserlib_BUILD_DOC</code></td>
<td>Enable / Disable documentation generation (requires a working [Doxygen](https://www.doxygen.nl/) installation).</td>
</tr>
<tr>
<td><code>mmtmhasparserlib_BUILD_BINARIES</code></td>
...
Find below the basic instructions to build the project or visit the ["Wiki Build"](https://github.com/Fraunhofer-IIS/mmtmhasparserlib/wiki/Build-instructions) page(s) to get further information (additional build parameters and platform specific build instructions).

1. Clone the project and make a build folder.
   ```
   $ git clone https://github.com/Fraunhofer-IIS/mmtmhasparserlib.git
   $ mkdir -p build
   ```
2. Configure the project using CMake.
   ```
   $ cmake -S mmtmhasparserlib -B build -DCMAKE_BUILD_TYPE=Release
   ```
3. Build the project.
   ```
